...
#'   Python tools directory containing MTL script templates.
#'
...
#' makeExec("/tmp/model/", 2, "ClassClass", "/path/to/tools/")
#' }
#'
#' @export
makeExec <- function(tmpDir, FFdepth, model_type, DEGAS.toolsPath) {
  if (
...
#' makeExec2("/tmp/densenet/", 3, "ClassClass", "/path/to/tools/")
#' }
#'
#' @export
makeExec2 <- function(tmpDir, FFdepth, model_type, DEGAS.toolsPath) {
  if (
    model_type != 'ClassClass' &&
      model_type != 'ClassCox' &&
      model_type != 'ClassBlank' &&
